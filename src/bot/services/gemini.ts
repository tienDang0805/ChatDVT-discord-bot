...
BẮT BUỘC TRẢ VỀ CHÍNH XÁC PHIÊN BẢN JSON NÀY (Không markdown, không bọc bởi \`\`\`):
{
  ...
  public async analyzeTarot(topic: string, question: string, drawnCards: { name: string; nameVi: string; position: string; isReversed: boolean }[], customApiKey?: string): Promise<any> {
      try {
          const cardsDescription = drawnCards.map(c => 
              `- Vị trí "${c.position}": Lá bài "${c.name}" (${c.nameVi}) — ${c.isReversed ? 'NGƯỢC (Reversed)' : 'XUÔI (Upright)'}`
          ).join('\n');

          const prompt = `Bạn là MỘT PHÁP SƯ TAROT HUYỀN BÍ bậc thầy, kết hợp tinh hoa Tarot phương Tây và trực giác phương Đông. Giọng văn huyền bí, sâu sắc, đôi khi hài hước nhẹ nhàng nhưng luôn mang tính tâm linh cao.

CHỦ ĐỀ BÓI: ${topic}
${question ? `CÂU HỎI CỤ THỂ: "${question}"` : 'Không có câu hỏi cụ thể — bói tổng quát theo chủ đề.'}

CÁC LÁ BÀI ĐÃ RÚT (Trải bài 3 lá: Quá Khứ — Hiện Tại — Tương Lai):
${cardsDescription}

QUY TẮC GIẢI BÀI:
1. Mỗi lá bài có 2 nghĩa: Xuôi (Upright) là năng lượng tích cực/thuận, Ngược (Reversed) là năng lượng bị chặn/ngược lại.
2. Giải nghĩa PHẢI phù hợp với VỊ TRÍ (Quá khứ → đã xảy ra, Hiện tại → đang diễn ra, Tương lai → sắp tới).
3. Liên kết 3 lá thành CÂU CHUYỆN MẠCH LẠC — chúng phải kể một hành trình có đầu có đuôi.
4. Nếu có câu hỏi cụ thể, hãy trả lời câu hỏi đó thông qua việc giải bài.
5. Viết bằng tiếng Việt, giọng huyền bí nhưng dễ hiểu. Có thể dùng thuật ngữ Tarot gốc tiếng Anh trong ngoặc.

BẮT BUỘC TRẢ VỀ ĐÚNG JSON (KHÔNG markdown, KHÔNG backtick):
{
  "overallReading": "<Tổng quan bức tranh 3 lá bài kể câu chuyện gì cho người rút — 4-5 câu, mạch lạc, sâu sắc>",
  "cards": [
    {
      "position": "<Quá Khứ / Hiện Tại / Tương Lai>",
      "interpretation": "<Giải nghĩa chi tiết lá bài ở vị trí này — 3-4 câu, tham chiếu đúng tên bài, xuôi/ngược, và chủ đề>",
      "energy": "<positive / negative / neutral>",
      "keywords": ["<3-4 từ khóa năng lượng của lá bài này>"]
    }
  ],
  "advice": "<Lời khuyên thực tế dựa trên 3 lá bài — 3-4 câu, mang tính hành động>",
  "luckyInfo": {
    "element": "<Nguyên tố chủ đạo (Lửa/Nước/Gió/Đất) dựa trên tổ hợp bài>",
    "color": "<Màu năng lượng may mắn>",
    "number": <1 con số may mắn 1-99>,
    "timing": "<Thời điểm tốt nhất để hành động, VD: 3 ngày tới / Tuần sau / Trăng mới tiếp theo>"
  },
  "spiritMessage": "<Thông điệp tâm linh sâu sắc 2-3 câu — như lời thì thầm từ vũ trụ gửi riêng cho người rút bài>"
}`;
          ...
          throw new Error(`Lỗi giải bài Tarot: ${error.message}`);
      }
  }
}
