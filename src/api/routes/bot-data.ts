...
router.post('/admin/reset-game/:userId', authenticateToken, async (req, res) => {
    try {
        ...
        await (prisma as any).userItemPurchaseLog.deleteMany({ where: { userId } });
        await (prisma as any).systemLog.deleteMany({ where: { level: 'journeylog', message: userId } });
        await prisma.userIdentity.update({ where: { userId }, data: { money: 0 } });
        userIdentityService.invalidateCache(userId);
        res.json({ success: true, message: 'Đã reset toàn bộ game data (Pet, Inventory, Cooldown, Expedition, Xu, Purchase Log, Journey Log).' });
    } catch (error) {
