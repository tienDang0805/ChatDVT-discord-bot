...
const EditableMarkdown = ({ content, onSave }: { content: string; onSave: (v: string) => void }) => {
  ...
  if (editing) {
    return (
      <div className="border border-orange-500/50 rounded-lg overflow-hidden bg-white dark:bg-[#131923]">
        ...
        <textarea value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={handleKeyDown} autoFocus className="w-full bg-transparent text-slate-800 dark:text-slate-200 font-mono text-[12px] leading-relaxed p-3 resize-none outline-none" style={{ height: `${Math.max(80, editValue.split('\n').length * 20 + 16)}px` }} />
      </div>
    );
  }
  ...
  return (
    <div className="relative group cursor-pointer rounded-lg hover:bg-slate-50/50 dark:hover:bg-[#1a2332]/50 transition-colors px-2 py-1 border border-transparent hover:border-slate-200/50 dark:hover:border-slate-700/50" onClick={handleStartEdit}>
      <button className="absolute top-1 right-1 z-10 p-1 bg-white dark:bg-[#1f2937] border border-slate-200 dark:border-slate-700 rounded-md text-slate-400 hover:text-orange-500 opacity-0 group-hover:opacity-100 transition-all shadow-sm" title="Edit"><Pencil size={10} /></button>
