      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '32px 16px' }}>
        ...
        <div style={{ background: '#111118', border: '1px solid rgba(99,102,241,.12)', borderRadius: 20, padding: '20px 20px 16px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: '#818cf8', textTransform: 'uppercase', letterSpacing: 2 }}>Ngôn ngữ</label>
            <div style={{ display: 'flex', background: '#0d0d1a', borderRadius: 10, border: '1px solid rgba(99,102,241,.15)', overflow: 'hidden' }}>
              {([['vi', '🇻🇳 Tiếng Việt'], ['en', '🇬🇧 English']] as const).map(([val, label]) => (
                <button key={val} onClick={() => setLanguage(val)} style={{ padding: '7px 14px', border: 'none', background: language === val ? 'rgba(99,102,241,.15)' : 'transparent', color: language === val ? '#a5b4fc' : '#4b5563', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all .2s' }}>{label}</button>
              ))}
            </div>
          </div>

          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#818cf8', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 8 }}>{language === 'en' ? 'Mood / Context' : 'Tâm trạng / Ngữ cảnh'}</label>
          <textarea value={context} onChange={e => setContext(e.target.value)} placeholder={language === 'en' ? "What's on your mind? Sad/happy/angry/lonely because..." : "Đang nghĩ gì? Đang buồn/vui/giận/cô đơn vì...?"} rows={3} disabled={loading} style={{ width: '100%', background: '#0d0d1a', border: '1px solid rgba(99,102,241,.2)', borderRadius: 12, padding: '14px 16px', color: '#e5e7eb', fontSize: 14, outline: 'none', resize: 'none', boxSizing: 'border-box', lineHeight: 1.6 }} />

          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8, marginBottom: 14 }}>
            {suggestions.map((s, i) => (
              <button key={i} onClick={() => setContext(s)} style={{ padding: '5px 10px', borderRadius: 8, border: '1px solid rgba(99,102,241,.15)', background: 'transparent', color: '#6b7280', fontSize: 11, cursor: 'pointer', transition: 'all .15s' }}>{s}</button>
            ))}
          </div>

          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#818cf8', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 8 }}>{language === 'en' ? 'Style' : 'Phong cách'}</label>
          ...
          <button onClick={generate} disabled={loading || !context.trim()} style={{ width: '100%', padding: '14px 0', borderRadius: 12, border: 'none', background: loading || !context.trim() ? 'rgba(99,102,241,.2)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: 'white', fontSize: 15, fontWeight: 700, cursor: loading || !context.trim() ? 'not-allowed' : 'pointer', opacity: loading || !context.trim() ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 14, letterSpacing: 1 }}>
            {loading ? <><Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> {language === 'en' ? 'Generating...' : 'Đang deep...'}</> : <><Sparkles size={18} /> {language === 'en' ? 'GENERATE' : 'TẠO STATUS'}</>}
          </button>
        </div>
        ...
        {statuses.length > 0 && (
          <div className="fade-up" style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(99,102,241,.5)', textTransform: 'uppercase', letterSpacing: 2 }}>{language === 'en' ? 'Results' : 'Kết quả'}</p>
              <button onClick={generate} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(99,102,241,.2)', background: 'transparent', color: '#818cf8', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}><RefreshCw size={12} /> {language === 'en' ? 'Regenerate' : 'Gen lại'}</button>
            </div>
                  ...
                  <div key={i} className="fade-up" style={{ animationDelay: `${i * 80}ms`, background: '#111118', border: '1px solid rgba(99,102,241,.1)', borderRadius: 16, padding: '16px 18px', position: 'relative', transition: 'border-color .2s' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {tc && <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6, background: 'rgba(251,191,36,.08)', color: '#fbbf24', border: '1px solid rgba(251,191,36,.2)' }}>{tc.icon} {tc.label}</span>}
                      </div>
